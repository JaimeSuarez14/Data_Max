...
function renderizarCarrito() {
  ...
  Object.entries(conteo).forEach(([id, cantidad]) => {
    ...
    tr.innerHTML = `
      ...
      <td class="text-center">${formatearPrecio(producto.precio)}</td>
      <td class="text-center">
        <a href="#" title="aumentar" class="btn btn-success btn-sm agregar-producto" data-id="${
          producto.id
        }"><i class="fa-solid fa-plus"></i></a>
        <span class="mx-2 fw-bold"> ${cantidad} </span>
        <a href="#" title="disminuir" class="btn btn-warning btn-sm disminuir-producto" data-id="${
          producto.id
        }"><i class="fa fa-minus"></i></a>
      </td>
      <td class="text-center">${formatearPrecio(importe)}</td>
      <td class="text-center"><a href="#" title="Eliminar" class="btn btn-danger btn-sm eliminar-producto" data-id="${
        producto.id
